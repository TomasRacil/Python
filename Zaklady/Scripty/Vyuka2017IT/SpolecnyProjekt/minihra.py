#zacitHru= input("Minihra přežít nástup začíná, jsi připraven? (Ano-A)\n")

#if zacitHru.lower().strip()=="a\n":
#print("Hra začíná")
import time
import random
print("Slova na úvod: Po těžké cestě na nástup kde už jsi zažil spustu trápení a nebezpečí tě čeká v tuto chvíli ještě ta nejtěžší část a to přežít nástup s Romanem")
seznam = ["Modrá podskupina","Bigoš","Pilot - odpad armády"]

k=random.choice(seznam)
print(f"Dnešní nástup budeš přežívat jako {k}\n")
...
if polozka in seznam == 'zelená'
		seznam_bigo = ["Na co čumíš,nervy moje","Co se to tam kurva vykecávate zase","Chceš do držky?"]
			print(f"V klidu si stojíš ve tvaru a povídáš si s chábrem vedle tebe když v tom Roman řve {random.choice(seznam_bigo)}")
			"""