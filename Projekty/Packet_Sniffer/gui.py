...
class GUI(Tk):
    ...
    def __init__(
        self,
        startSniffing: Callable,
        stopSniffing: Callable,
    ...
    def updatePackets(self):
        ...
        try:
            ...
            if packet[7] != "":
                self._add_record_to_vuln_window(
                    vulnRecordWrapper, packet[:: len(packet) - 1]
                )
        except Empty:
            pass
    ...
    def _openDialog(self):
        ...
        fileName = filedialog.askopenfilename(
            ...
            title="Select A Capture(CSV file)",
            filetypes=[("CSV", "*.csv")],
        )
        if len(fileName) != 0:
            pass
            ...
            [
                self._add_record_to_packet_window(recordWrapper, row)
                for row in self._loadedPacketsFromCSV
            ]
            vulnList = [
                row[7:] for row in self._loadedPacketsFromCSV if row[7:][1].strip()
            ]

            if vulnList:
                [
                    self._add_record_to_vuln_window(
                        self._snifferVulnRecordsWrapperFrame, row
                    )
                    for row in vulnList
                ]
    ...
    def _create_control_menu(self):
        ...
        self.startButton = Button(
            controlsFrame, text="Start", command=lambda: self.startCapturingButtonPush()
        )
        self.stopButton = Button(
            controlsFrame, text="Stop", command=lambda: self.stopCapturingButtonPush()
        )
        self.stopButton["state"] = "disabled"
    ...
    def _create_packets_window(self):
        ...
        packetNumberLabel = Label(
            snifferHeaderFrame, text="No.", bg="white", width=10, anchor="w"
        )
        timestampLabel = Label(
            timestampFrame, text="Time", width=14, bg="white", anchor="w"
        )
        sourceLabel = Label(
            snifferHeaderFrame, text="Source", bg="white", width=14, anchor="w"
        )
        destinationLabel = Label(
            snifferHeaderFrame, text="Destination", bg="white", width=14, anchor="w"
        )
        protocolLabel = Label(
            snifferHeaderFrame, text="Protocol", bg="white", width=12, anchor="w"
        )
        packetLengthLabel = Label(
            snifferHeaderFrame, text="Length", bg="white", width=12, anchor="w"
        )
        packetBodyLabel = Label(
            snifferHeaderFrame, text="Info", bg="white", width=77, anchor="w"
        )
    ...
    def _create_vulnerabilities_window(self):
        ...
        snifferVulnerabilitesHeaderFrame = Frame(
            snifferVulnWrapperFrame, padx=5, pady=5, bg="white"
        )
        ...
        packetNumberVulnLabel = Label(
            snifferVulnerabilitesHeaderFrame,
            text="No.",
            bg="white",
            width=10,
            anchor="w",
        )
        packetBodyVulnLabel = Label(
            snifferVulnerabilitesHeaderFrame,
            text="Scraped Data",
            bg="white",
            width=148,
        ...
        self._snifferVulnRecordsWrapperFrame = Frame(
            snifferVulnerabilitesFrame.scrolled_frame
        )
