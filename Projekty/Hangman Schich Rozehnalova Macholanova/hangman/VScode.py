...
# Funkce je zavolána, pokud uživatel správně uhodne písmeno. Kraslí písmeno
def drawLetter(word, character):
    global count
    # Creates list of when 'character' appears in 'word'
    ...
    for i in range(len(times)):  # Draws character based on when it appears in 'word'
        ...
        t.write(character, move=True, align='center',
...
def guess():  # Uživatelský vstup
    ...
    global n
    global mistake
    # ptáme se přes 'message box'
    character = t.textinput('Tvůj tip', 'Tipni si písmeno\'')
                ...
                try:
                    if Password == data[Username]:
                        ...
                        messagebox.showerror(
                            "Chyba", "Heslo nebo jméno není správné")
                        access()
                except:
...
def score_fnce():
        ...
        if name == user:
            ...
            for line in lines:  # pro jeden řádek v řádcích
                if user in line:  # pokud je uživatelovo jméno v řádku
                    txt = line
                    # opravono takhle, kvůli více ciferným číslům
                    num = [int(i) for i in txt.split() if i.isdigit()]
                    # pocet her ++
    ...
    # xt pokud soubor neexistuje, vytvoří se nový, otevřeno jen pro zápis
    with open("zebricek.txt", "xt") as leaderboard:
...
def fake_main():
    ...
    while not done:
        if n > 5:
            ...
            t.penup()
            t.setpos(50, 70)
            t.write('Prohra.' + 'Slovo bylo \'' + word + '\'',
                    move=True, align='center', font=('Arial', 10, 'normal'))
        elif count > len(word) - 2:  # -> upravuje delku slova o -2
            done = True
            t.penup()
            t.setpos(50, 70)
            t.write('Super! Uhodl jsi slovo \'' + word + '\'. Vyhrál jsi!',
                    move=True, align='center', font=('Arial', 10, 'normal'))
        else:
            guess()
    return None
    ...
    while not konec:
        with open("slovnik.txt", encoding="utf-8") as soubor:
            ...
            while x == 2:
                ...
                if (len(word) - 1) >= 6 and (len(word) - 1) <= 8 and ("x" not in word) and ("q" not in word):
