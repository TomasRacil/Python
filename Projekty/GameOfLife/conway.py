...
def main():
    ...
    if polygon or ellipse is True:
        ...
        if polygon is True:
            poly = generatePolygon( ctrX=N/2, ctrY=N/2, aveRadius=N/3, irregularity=0.35, spikeyness=0.2, numVerts=verts )
        else: 
            ell = Ellipse((N/2, N/2), e_width, e_height, 120, ec='b', fill = False)
            poly = ell.get_verts()
        
        geom_pts = gen_point_in_polygon(Polygon(poly)) # <shapely.geometry.point.Point object at...>
