def instadownloadstart():
    ...
    print(
        "Pokud je profil soukromý, zadejte své přihlašovací jméno, pokud je veřejný, stikněte ENTER."
    )
    ...
    if len(USER) == 0:
        ...
        PASSWORD = getpass.getpass(
            "Zadejte své heslo:"
        )  # Použití getpass pro skrytí hesla v konzoli
        Name = "{target}_{mediaid}"

    # Začátek podprogramu pro stahování a zadávání do databáze(běží ve smyčce)
    def function():
        """
        Po zadání přihlašovacího jména stáhne všechny posty a metadata, následně je extrahuje z .xz formátu a extrahovaná data uloží do tabulek users a posts, poté se sama znovu spustí
        """
        ...
        for post in profile.get_posts():
            ...
            filename = (
                profile.username
                + "/"
                + L.format_filename(post, target=profile.username)
            )
        ...
        for file_xz in glob.glob("*.xz"):
            ...
            patoolib.extract_archive(
                ...
                + file_xz[s:e]
                + ".json.xz",
                outdir=cesta + "/data/" + PROFILE + "/",
            )  # extrahování .xz souborů pomocí knihovny patoolib
            ...
            popisek = (
                ...
                + file_xz[s:e]
                + ".txt."
            )  # popisek je vložen pouze jako cesta k .txt souboru, protože když jsem chtěl vložit přímo text, tak byl problém s různými emotikony a znaky, kterých je na instagramu poměrně hodně
            id_post = data["node"]["id"]
            display_url = data["node"]["display_url"]
            edge_liked_by = data["node"]["edge_media_preview_like"]["count"]
            followers = data["node"]["owner"]["edge_followed_by"]["count"]
            followed = data["node"]["owner"]["edge_follow"]["count"]
            zapis1 = "INSERT INTO posts(ID, id_post, posted_by, display_url, edge_liked_by, popisek) VALUES(%s, %s, %s, %s, %s, %s)"
            record1 = (0, id_post, PROFILE, display_url, edge_liked_by, popisek)
